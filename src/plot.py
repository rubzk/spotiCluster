...
class Plot3D:
    ...
    def scatter_3d(self, x_ax, y_ax, z_ax):
        fig = px.scatter_3d(self.df, x=x_ax, y=y_ax, z=z_ax, color='cluster', hover_data=['song_name'], width=600,
    ...
    def radar_chart(self):
        ...
        for i in range(0, self.n_clusters):
            fig.add_trace(go.Scatterpolar(
                r=[self.cluster_stats.loc[i]['danceability'], self.cluster_stats.loc[i]['energy'],
                   self.cluster_stats.loc[i]['loudness'],
                   self.cluster_stats.loc[i]['speechiness'], self.cluster_stats.loc[i]['acousticness'],
                   self.cluster_stats.loc[i]['instrumentalness'],
                   self.cluster_stats.loc[i]['liveness'], self.cluster_stats.loc[i]['valence'],
                   self.cluster_stats.loc[i]['tempo']],
                theta=categories,
                fill='toself',
                name=f'Cluster {i}'
            ))
